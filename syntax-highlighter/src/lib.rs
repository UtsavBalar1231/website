...
use tokenizer::{Tokenizer, Language, Token, TokenType};
...
fn extract_language_from_class(class_name: &str) -> Option<String> {
    for class in class_name.split_whitespace() {
        if let Some(lang) = class.strip_prefix("language-") {
            return if lang.is_empty() { None } else { Some(lang.to_string()) };
        }
  ...
  web:
    ...
    ports:
      - "80:80""#),
        ...
        for (lang, code) in test_cases {
            ...
            assert!(result.contains("<span"), "No spans found for language: {}", lang);
            assert!(result.contains("hl-"), "No highlighting classes for language: {}", lang);
            
            // Should escape HTML properly
            if code.contains("<") {
                assert!(result.contains("&lt;"), "HTML not escaped for language: {}", lang);
            }
        }
    }
    ...
    fn test_html_injection_prevention() {
        ...
        for input in malicious_inputs {
            ...
            assert!(!result.contains("<script>"), "Script tag not escaped: {}", input);
            assert!(!result.contains("javascript:"), "JavaScript URL not escaped: {}", input);
            assert!(!result.contains("onerror="), "Event handler not escaped: {}", input);
    ...
    fn test_language_extraction_edge_cases() {
        ...
        for (input, expected) in test_cases {
            assert_eq!(extract_language_from_class(input), expected, "Failed for input: '{}'", input);
        }
    }
...
"#;
    ...
    fn test_language_aliases() {
        ...
        for alias in c_aliases {
            ...
            assert!(result.contains("hl-keyword"), "Failed for C alias: {}", alias);
        }
        ...
        for alias in bash_aliases {
            ...
            assert!(result.contains("hl-keyword"), "Failed for Bash alias: {}", alias);
        }
    }
